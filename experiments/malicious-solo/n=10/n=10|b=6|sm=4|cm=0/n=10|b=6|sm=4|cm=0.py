...
# df = pd.DataFrame({entry["metric"]: f"{entry['mean']} ({entry['std']})"
#                    for entry in data}, index=["malicious-collaborative, n=100"])

# # Format the DataFrame using tabulate
# table_format = tabulate(df, headers='keys', tablefmt='plain', showindex=True)

# print(table_format)

# Example data for multiple rows
# data_row1 = [
#     {"metric": "avg_correction_term_normal_bidders", "mean": 0.562, "std": 0.042},
#     {"metric": "optimality", "mean": 0.05, "std": 0.056},
#     {"metric": "efficiency", "mean": 0.011, "std": 0.022},
#     {"metric": "avg_correction_term_malicious_bidder", "mean": 0.259, "std": 0.038},
#     {"metric": "malicious", "mean": 0.604, "std": 0.494}
# ]

# data_row2 = [
#     {"metric": "avg_correction_term_normal_bidders", "mean": 0.512, "std": 0.032},
#     {"metric": "optimality", "mean": 0.045, "std": 0.051},
#     {"metric": "efficiency", "mean": 0.009, "std": 0.018},
#     {"metric": "avg_correction_term_malicious_bidder", "mean": 0.239, "std": 0.034},
#     {"metric": "malicious", "mean": 0.564, "std": 0.412}
# ]

# data_row3 = [
#     {"metric": "avg_correction_term_normal_bidders", "mean": 0.524, "std": 0.037},
#     {"metric": "optimality", "mean": 0.047, "std": 0.053},
#     {"metric": "efficiency", "mean": 0.010, "std": 0.021},
#     {"metric": "avg_correction_term_malicious_bidder", "mean": 0.261, "std": 0.036},
#     {"metric": "malicious", "mean": 0.591, "std": 0.472}
# ]

# # Create a list of dictionaries for each row of data
# all_data = [data_row1, data_row2, data_row3]

# # Create a list of DataFrame for each row of data
# dfs = []
# for i, data_row in enumerate(all_data):
#     metric_data = {}
#     for entry in data_row:
#         metric = entry["metric"]
#         metric_data[metric] = f"{entry['mean']} ({entry['std']})"
#     df = pd.DataFrame(metric_data, index=[f"Row {i+1}"])
#     dfs.append(df)
...
# # Format the DataFrame using tabulate
# table_format = tabulate(combined_df, headers=['h','a','l','l','o'], tablefmt='plain', showindex=True)

# # Print or save the formatted table
# print(table_format)
